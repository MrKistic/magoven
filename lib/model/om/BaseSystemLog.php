...
/**
 * Base class that represents a row from the 'system_log' table.
 *
 * 
 *
 ...
 */
abstract class BaseSystemLog extends BaseObject  implements Persistent {
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = SystemLogPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = SystemLogPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function setLogType($v)
	{
		...
		if ($this->log_type !== $v) {
			$this->log_type = $v;
			$this->modifiedColumns[] = SystemLogPeer::LOG_TYPE;
		}
	...
	public function setApplicationId($v)
	{
		...
		if ($this->application_id !== $v) {
			...
			$this->modifiedColumns[] = SystemLogPeer::APPLICATION_ID;
		}
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = SystemLogPeer::USER_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->log_type = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->message = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->application_id = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			...
			return $startcol + 7; // 7 = SystemLogPeer::NUM_COLUMNS - SystemLogPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseSystemLog:delete:pre') as $callable)
			{
			...
			if ($ret) {
				SystemLogPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseSystemLog:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseSystemLog:save:pre') as $callable)
			{
			...
			if ($this->isModified() && !$this->isColumnModified(SystemLogPeer::UPDATED_AT))
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(SystemLogPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseSystemLog:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SystemLogPeer::doInsert($this, $con);
					...
					$affectedRows += SystemLogPeer::doUpdate($this, $con);
				}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setLogType($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setMessage($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setApplicationId($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SystemLogPeer::CREATED_AT)) $criteria->add(SystemLogPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(SystemLogPeer::UPDATED_AT)) $criteria->add(SystemLogPeer::UPDATED_AT, $this->updated_at);
		if ($this->isColumnModified(SystemLogPeer::LOG_TYPE)) $criteria->add(SystemLogPeer::LOG_TYPE, $this->log_type);
		if ($this->isColumnModified(SystemLogPeer::MESSAGE)) $criteria->add(SystemLogPeer::MESSAGE, $this->message);
		if ($this->isColumnModified(SystemLogPeer::APPLICATION_ID)) $criteria->add(SystemLogPeer::APPLICATION_ID, $this->application_id);
		if ($this->isColumnModified(SystemLogPeer::USER_ID)) $criteria->add(SystemLogPeer::USER_ID, $this->user_id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseSystemLog:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseSystemLog::%s', $method));
	  }
