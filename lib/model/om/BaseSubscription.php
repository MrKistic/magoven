...
abstract class BaseSubscription extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the last_validated field.
	 * @var        string
	 */
	protected $last_validated;
	...
	public function getEffectiveDate($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->effective_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
	...
	/**
	 * Get the [optionally formatted] temporal [last_validated] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getLastValidated($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->last_validated === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->last_validated);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->last_validated, true), $x);
			}
		}
	...
	public function setApplicationId($v)
	{
		...
		if ($this->application_id !== $v) {
			...
			$this->modifiedColumns[] = SubscriptionPeer::APPLICATION_ID;
		}
	...
	public function setEffectiveDate($v)
	{
		...
		if ( $this->effective_date !== null || $dt !== null ) {
			...
			$currNorm = ($this->effective_date !== null && $tmpDt = new DateTime($this->effective_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->effective_date = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = SubscriptionPeer::EFFECTIVE_DATE;
			}
		} // if either are not null
	...
	public function setExpirationDate($v)
	{
		...
		if ( $this->expiration_date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = SubscriptionPeer::EXPIRATION_DATE;
			}
		} // if either are not null
	...
	/**
	 * Sets the value of [last_validated] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Subscription The current object (for fluent API support)
	 */
	public function setLastValidated($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->last_validated !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->last_validated !== null && $tmpDt = new DateTime($this->last_validated)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->last_validated = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = SubscriptionPeer::LAST_VALIDATED;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->effective_date = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->expiration_date = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->last_validated = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
			...
			return $startcol + 8; // 8 = SubscriptionPeer::NUM_COLUMNS - SubscriptionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[5] => $this->getEffectiveDate(),
			$keys[6] => $this->getExpirationDate(),
			$keys[7] => $this->getLastValidated(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setEffectiveDate($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setExpirationDate($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setLastValidated($arr[$keys[7]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SubscriptionPeer::APPLICATION_ID)) $criteria->add(SubscriptionPeer::APPLICATION_ID, $this->application_id);
		if ($this->isColumnModified(SubscriptionPeer::USER_ID)) $criteria->add(SubscriptionPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(SubscriptionPeer::EFFECTIVE_DATE)) $criteria->add(SubscriptionPeer::EFFECTIVE_DATE, $this->effective_date);
		if ($this->isColumnModified(SubscriptionPeer::EXPIRATION_DATE)) $criteria->add(SubscriptionPeer::EXPIRATION_DATE, $this->expiration_date);
		if ($this->isColumnModified(SubscriptionPeer::LAST_VALIDATED)) $criteria->add(SubscriptionPeer::LAST_VALIDATED, $this->last_validated);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setLastValidated($this->last_validated);
